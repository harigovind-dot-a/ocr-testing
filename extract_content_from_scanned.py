...
def analyze_batch(images, start_page_num):
    ...
    user_content = [
        {
            ...
            "text": (
                ...
                "3. **Return a JSON object** with a 'matches' list. Each match must look like this:\n"
                "{ 'page': int, 'section_detected': string, 'confidence': string }"
            )
        }
    ]
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are an Educational Content Extractor. You are highly accurate at reading textbook headings."},
        ...
        valid_pages = []
        for m in matches:
            print(f"  -> Found Page {m['page']}: {m['section_detected']} (Confidence: {m.get('confidence', 'N/A')})")
